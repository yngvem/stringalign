...
def test_replace_instantiation(predicted: str, reference: str) -> None:
    """It should be possible to instantiate the Replaced class with a predicted and a reference text"""
    replaced = Replaced(predicted, reference)
    assert replaced.predicted == predicted
    assert replaced.reference == reference


@given(predicted=st.text(), reference=st.text())
def test_replace_equality(predicted: str, reference: str) -> None:
    """Two Replaced objects created with the same predictions and references should be equal"""
    replace1 = Replaced(predicted=predicted, reference=reference)
    replace2 = Replaced(predicted=predicted, reference=reference)
    assert replace1 == replace2


@given(predicted=st.text(), reference=st.text())
def test_replace_replaces_with_itself(predicted: str, reference: str) -> None:
    """Generalizing a Replaced object returns itself"""
    replaced = Replaced(predicted=predicted, reference=reference)
...
def test_replace_with_emtpy_string_simplifies_to_delete(predicted: str) -> None:
    ...
    replaced = Replaced(predicted=predicted, reference="")
...
def test_replacing_empty_string_simplifies_to_insert(reference: str) -> None:
    ...
    replaced = Replaced(predicted="", reference=reference)
    simplified_replaced = replaced.simplify()
...
def test_replace_inequality_predicted(predicteds: tuple[str, str], reference: str) -> None:
    ...
    replace1 = Replaced(predicted=predicted1, reference=reference)
    replace2 = Replaced(predicted=predicted2, reference=reference)
    assert replace1 != replace2
...
def test_replace_inequality_reference(predicted: str, references: tuple[str, str]) -> None:
    ...
    replace1 = Replaced(predicted=predicted, reference=reference1)
    replace2 = Replaced(predicted=predicted, reference=reference2)
    assert replace1 != replace2
