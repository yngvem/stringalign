...
def test_cost_matrix_shape(reference, predicted):
    cost_matrix = create_cost_matrix(reference, predicted)
    assert cost_matrix.shape == (len(reference) + 1, len(predicted) + 1)


@given(st.text(), st.text())
def test_cost_matrix_first_row_and_column(reference, predicted):
    cost_matrix = create_cost_matrix(reference, predicted)
...
@given(st.text(), st.text())
def test_cost_matrix_values(reference, predicted):
    cost_matrix = create_cost_matrix(reference, predicted)
...
@given(st.text())
def test_cost_matrix_identical_strings(text):
    cost_matrix = create_cost_matrix(text, text)
