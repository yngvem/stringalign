...
def test_simple_example() -> None:
    ...
    assert evaluator.false_negative_index["a"] == frozenset(
        {evaluator.alignment_analyzers[0], evaluator.alignment_analyzers[2]}
    )
    assert evaluator.false_negative_index["f"] == frozenset({evaluator.alignment_analyzers[1]})
...
def test_key_error_for_non_existent_false_negative() -> None:
    ...
    with pytest.raises(KeyError):
        _ = evaluator.false_negative_index["c"]
